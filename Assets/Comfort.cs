...
public class Comfort
{
    ...
    /// <param name="DebugMode">Sollen Debug Informationen ausgegeben werden</param>
    /// <returns>Gibt an ob ein Feld angegriffen wird</returns>
    public int AnzahlAngegriffen(Situation sit, int[] Feld, int VonSpieler, bool Schachprüfen)
    {
        Situation WorkWithMe = sit.Clone();
        WorkWithMe.SpielerID = VonSpieler;
        int ret = 0;
        for (int i = 0; i < 8; i++)
        {
            for (int j = 0; j < 8; j++)
            {
                if (ZugLegal(WorkWithMe, new Zug(new int[] { i, j }, Feld, "D"), Schachprüfen, false))
                {
                    ret++;
                }
            }
    ...
    /// <param name="Schachprüfen">Sollen nur Züge zurückgegeben werden, nach denen der Spieler nicht im Schach steht?</param>
    /// <returns>Ein Array der möglichen Züge.</returns>
    public Zug[] AlleMöglichenZüge(Situation sit)
    {
        return AlleMöglichenZüge(sit, true);
    }
